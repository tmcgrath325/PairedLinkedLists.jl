@testset "PairedLinkedList" begin
    ...
    @testset "core functionality" begin
            ...
            @testset "push back" begin
                for i = 1:n
                    ...
                    if i > 3
                        ...
                        io = IOBuffer()
                        @test sprint(io -> show(io, iterate(l1))) == "(PairedListNode{Int32}(1), PairedListNode{Int32}(2))"
                        @test sprint(io -> show(io, iterate(l1, l1.head.next.next))) == "(PairedListNode{Int32}(2), PairedListNode{Int32}(3))"
                    end
                    cl = collect(l)
        ...
        #         @testset "map" begin
        #             l = PairedLinkedList{Int}(1:n...)
        #             @test map(x -> 2x, l) == PairedLinkedList{Int}(2:2:2n...)
        #             l2 = PairedLinkedList{Float64}()
        #             @test map(x -> x*im, l2) == PairedLinkedList{Complex{Float64}}()
        #             @test map(Int32, l2) == PairedLinkedList{Int32}()
        #             f(x) = x % 2 == 0 ? convert(Int8, x) : convert(Float16, x)
        #             @test typeof(map(f, l)) == PairedLinkedList{Real}
        #         end
