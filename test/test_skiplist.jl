@testset "SkipList" begin
    ...
    @testset "core functionality" begin
        ...
        # @testset "push / pop back" begin
        #     l = SkipList{Int}()
        #     dummy_list = SkipList{Int}()
        #     @test_throws ArgumentError insertafter!(newnode(dummy_list, 0), l.head)

        #     @testset "push back" begin
        #         for (i,val) in enumerate(n:-1:i)
        #             push!(l, val)
        #             @test first(l) == (val)
        #             @test getindex(l, i) == n+1-i
        #             @test getindex(l, 1:floor(Int, i/2)) == SkipList{Int}(1:floor(Int, i/2)...)
        #             @test l[1:floor(Int, i/2)] == SkipList{Int}(1:floor(Int, i/2)...)
        ...
        #                 io = IOBuffer()
        #                 @test sprint(io -> show(io, iterate(l1))) == "(1, SkipNode{Int32, SkipList{Int32}}(2))"
        #                 @test sprint(io -> show(io, iterate(l1, l1.head.next.next))) == "(2, SkipNode{Int32, SkipList{Int32}}(3))"
        #             end
        #             cl = collect(l)
